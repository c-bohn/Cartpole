...
class ReplayMemory():
    ...
    def sample_batch(self):
        ...
        #TODO just a test to see if the model can overfit a single batch
        #sample = list(self.memory)[:BATCH_SIZE]
...
#Working configuration: Achieved 500 steps for some games but not all
#Learning rate: lr = (1e-3 / (1.+1e-3*num_episodes_done)) + 1e-5
#Exploration rate: exploration_rate = max(0.05, 0.5-0.0001*num_episodes_done)
class DQN(tf.keras.Model):
    ...
    def call(self, x):
        ...
        x = self.bn_6(x)
        x = self.fc_3(x)
        pred_state = x #Not used in this version
...
#        self.dqn.load_weights('./checkpoints/my_checkpoint')
#        print("restored model")
#        self.target_dqn.load_weights('./checkpoints/my_checkpoint')
#        ##########
        ...
        while num_episodes_done < NUM_EPISODES:
            ...
            while not done:
                ...
                if random.random() < exploration_rate and num_episodes_done%10 != 9:
                    ...
                    _, prediction = self.dqn(tf.reshape(prev2_prev_curr_next_screen[:, :, :3], [1, *screen_size]))
...
#                input("...")
    ...
    def inference(self, num_games):
        ckpt = './checkpoints_episode_9300/my_checkpoint'
